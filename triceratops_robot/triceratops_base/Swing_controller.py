...
class SwingController:
    ...
    def raibert_touchdown_location(self, leg_index, command):
        ...
        if not np.all(delta_p == 0):
            if 0 <= leg_index < 4: 
                self.com[0, leg_index] = - command.horizontal_velocity[0] / 3
                self.com[1, leg_index] = - command.horizontal_velocity[1] / 3
        else:
            self.com[0, leg_index] = self.com[1, leg_index] = 0
        ...
        if command.yaw_rate != 0:
            if 0 <= leg_index < 4:
                self.com[:, leg_index] = 0  # Reset COM if yaw rate is non-zero
                delta_p = np.array([0, 0, 0])

        return R @ (self.config.default_stance[:, leg_index] + self.com[:, leg_index]) + delta_p
